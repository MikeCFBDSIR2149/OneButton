...
public class SliceObject : MonoBehaviour
{
    ...
    public void Slice(Vector2 startPos, Vector2 endPos)
    {
        ...
        var preVertCount = vertList.Count;
        ...
        for (var i = 0; i < triList.Count; i += 3)
        {
            var triIndex0 = triList[i];
            var triIndex1 = triList[i + 1];
            var triIndex2 = triList[i + 2];
        ...
        {
            float mx = 0, mn = 1;
            for (var i = 0; i < CrossVertList.Count; i++)
            {
                Vector2 v = CrossVertList[i];
                var relate = GetRelate(startPos, endPos, v);
                if (relate > mx)
                {
                    mx = relate;
                    farestPointIndex = i;
        ...
        var crossProduct = toZeroFar.x * toZeroNear.y - toZeroFar.y * toZeroNear.x;
        var corrective = crossProduct < 0;
        if (corrective) (nearestPointIndex, farestPointIndex) = (farestPointIndex, nearestPointIndex);
        ...
        var LeftPos = Consts2D.IsPointOnLeftSideOfLine(vertList[0], startPos, endPos);
        var ExceedLineTime = 0;
        LeftMesh.AddVert(vertList[0], uvList[0]);
        for (var i = 1; i < preVertCount; ++i)
        {
            var Pos = Consts2D.IsPointOnLeftSideOfLine(vertList[i], startPos, endPos);
            // 第一次见到
            if (Pos != LeftPos)
            {
                // 初次越界
        ...
        rightGo.transform.position -= (Vector3)normalline / 100;
        transform.position += (Vector3)normalline / 100;
    ...
    public static Vector2 GetUVPoint(in Vector2 startUV, in Vector2 endUV, in Vector2 startPoint, in Vector2 endPoint,
        Vector2 curPoint)
    {
        // 计算出中间点相对起点的距离
        var relate = GetRelate(startPoint, endPoint, curPoint);
    ...
    {
        var relate = (startPoint - curPoint).magnitude / (startPoint - endPoint).magnitude;
        return relate;
    }
    ...
    private static bool IsEffectiveCutting(List<int> triList, List<Vector3> vertList, in Vector2 startPos,
        in Vector2 endPos)
    {
        ...
        for (var i = 0; i < triList.Count; i += 3)
        {
            ...
            if (Consts2D.HasCrossPoint(startPos, endPos, point0, point1, out _) ||
                ...
                Consts2D.HasCrossPoint(startPos, endPos, point2, point0, out _)) isEffective = true;
...
public class OldMeshData
{
    ...
    public void GenTri()
    {
        for (var i = 1; i < VertList.Count - 1; i++)
        {
            TriList.Add(0);
            TriList.Add(i);
            TriList.Add(i + 1);
